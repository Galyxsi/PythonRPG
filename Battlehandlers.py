...
class Battle:
    ...
    def load_available_movement(self, map):
        ...
        available_tiles = []
        max_steps = current_char.movement_speed // 5
    ...
    def render(self, surface, camera, frame):
        available_tile = pygame.image.load("sprites/tilesets/available_tile.png").convert_alpha()
        for tile in self.available_tiles:
            surface.blit(available_tile, (tile[0] * 16 - camera[0], tile[1] * 16 - camera[1]))
    ...
    def move_character(self, pos, map):
        if pos in self.available_tiles:
            ...
            self.character_paths[self.character_list[self.char_turn]] = Chr.Pathfinder.Pathfind((math.ceil((self.character_list[self.char_turn].x + 0.5) // 16), math.ceil((self.character_list[self.char_turn].y + 0.5) // 16)), (pos[0], pos[1]), map, "4-dir")
